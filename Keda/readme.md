...
- [Deep dive into the scaling](#deep-dive-into-the-scaling)
  - [Evaluating scaling algorithm](#evaluating-scaling-algorithm)
...
## Evaluating scaling algorithm

The [keda.yaml](./keda.yaml) has `targetValue` set on 2. It maps to `averageValue` set on 2 in the HPA generated by the keda. It means that `desiredMetricValue` is 2 because `desiredMetricValue` is the value defined in the `yaml` file.
Following this algorithm explained above *desiredReplicas = ceil[currentReplicas * ( currentMetricValue / desiredMetricValue )]* we can compute these values:
